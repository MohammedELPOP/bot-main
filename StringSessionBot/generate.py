...
async def main(_, msg):
    await msg.reply(
        "¤¦ يرجى تحديد نوع الجلسة المراد استخراجها",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("¤¦ كود بـايروجرام", callback_data="pyrogram"),
            InlineKeyboardButton("¤¦ كود ترمكس", callback_data="telethon")
        ]])
    )
...
🔰 اضـغـط /skip لـلـتـخـطـي', filters=filters.text)
    ...
    try:
        ...
        return
    api_hash_msg = await bot.ask(user_id, '🔰 اࢪسـل الان ايبي ايدي `API_HASH`

🔰 اضـغـط /skip لـلـتـخـطـي', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    api_hash = api_hash_msg.text
    phone_number_msg = await bot.ask(user_id, '- الان يرجى ارسال `PHONE_NUMBER` مع رمز الدوله . \nمثال : `+9647806776435`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\من قبل  @sessions78".format("TELETHON" if telethon else "PYROGRAM", string_session)
    try:
        await client.send_message("me", text)
    except KeyError:
        pass
    await client.disconnect()
    await phone_code_msg.reply("Successfully generated {} string session. \n\nPlease check your saved messages! \n\nBy @sessions78".format("telethon" if telethon else "pyrogram"))
